...
make ci          # what CI checks on a pull request: gofmt, lint, actionlint, tests, govulncheck, docs build
```

`make ci` leaves out the slow jobs; `make ci-full` adds the build with the minimum Go version, 30 seconds of fuzzing per target and the release snapshot. The `.deb` checks (lintian, installing with apt) only run in CI.
