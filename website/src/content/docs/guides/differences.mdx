...
To have, say, one `alacritty.toml` on your Macs and another on your Linux machines, use two entries with the **same target**, different sources, and one OS each:
...
Each machine only manages the entry for its own OS, so the two never meet. `dotsync add` refuses a second entry for a target that's already managed, even for another OS, so create the first entry with `dotsync add … --source alacritty/macos.toml --os darwin` on a Mac, then add the second [by hand](/dotsync/guides/manage-files/#edit-the-manifest-by-hand) in a clone of your repository and push. The first Linux machine to sync uploads its file.
