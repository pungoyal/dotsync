...
You can also edit `manifest.json` directly in any clone of your dotfiles repository, commit and push. That's handy for bulk changes, and for the few things the commands don't do, such as [two entries for the same path on different systems](/dotsync/guides/differences/#a-different-file-per-os). Machines pick the change up on their next sync. The [manifest reference](/dotsync/reference/manifest/) lists every field.
