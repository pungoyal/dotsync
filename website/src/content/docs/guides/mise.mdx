...
Copy it to each new machine yourself, once. Until you do, `dotsync doctor` on that machine warns about any age or sops files it can't find a key for. It doesn't read mise's own config syntax, so it can't tell you about values encrypted with `mise set --age-encrypt`; those show up as missing variables in `mise env` instead:
