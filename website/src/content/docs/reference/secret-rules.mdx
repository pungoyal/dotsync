...
The age **private key** that decrypts them is always refused, by its path and by its content (`AGE-SECRET-KEY-1…`). Copy it to each machine yourself.
