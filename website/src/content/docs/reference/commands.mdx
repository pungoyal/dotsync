...
Sets up this machine. It clones the repository into `~/.local/share/dotsync/repo` and, if the repository is empty, creates the manifest there and pushes it. Then it writes the [machine configuration](/dotsync/reference/configuration/), copies the running binary to `~/.local/bin/dotsync`, runs the first sync, and installs the [background agent](/dotsync/guides/background-agent/).
