...
func cmdSet(args []string, stdout, stderr io.Writer) (int, error) {
	...
	var f setFlags
	fs.StringVar(&f.desc, "d", "", "describe the entry with `text`")
	fs.StringVar(&f.desc, "description", "", "same as -d")
	fs.Var(&f.addIgnore, "ignore", "add an ignore `glob` (repeatable)")
	fs.Var(&f.removeIgnore, "unignore", "remove an ignore `glob` (repeatable)")
	fs.BoolVar(&f.clearIgnore, "clear-ignore", false, "remove every ignore pattern")
	fs.Var(&f.oses, "os", "only manage the entry on this `OS`: darwin or linux (repeatable; replaces the current list)")
	fs.BoolVar(&f.anyOS, "any-os", false, "manage on every OS (removes the os restriction)")
	fs.StringVar(&f.mode, "mode", "", "force these octal permissions (`mode`, e.g. 0600) on every machine")
	fs.BoolVar(&f.noMode, "no-mode", false, "stop forcing permissions")
	fs.StringVar(&f.write, "write", "", "how local files are replaced (`atomic|inplace`)")
